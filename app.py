...
import ssl  # Import SSL module for options
...
# Function to try multiple MongoDB connection configurations
def try_connect_mongodb(mongo_uri):
    if not mongo_uri:
    ...
    configs = [
        ...
        {
            "name": "No certificate verification",
            "options": {
    ...
    for config in configs:
        try:
            print(f"Trying MongoDB connection with config: {config['name']}")
            client = pymongo.MongoClient(mongo_uri, **config["options"])
            
            # Test the connection
            client.admin.command('ping')
            print(f"✅ MongoDB connection successful with config: {config['name']}")
            return client, True
            
        except Exception as e:
            print(f"❌ MongoDB connection failed with config {config['name']}: {str(e)}")
...
try:
    ...
    if MONGO_URI:
        ...
        client, connection_success = try_connect_mongodb(MONGO_URI)
        
        if connection_success:
            db = client["tr_calculator"]
            ...
            print("⚠️ MongoDB connection failed after trying multiple configurations, using fallback storage")
            using_mongodb = False
    else:
        print("⚠️ No MongoDB URI provided, using fallback storage")
        using_mongodb = False
except ImportError as e:
    print(f"⚠️ Module import error: {str(e)}, using fallback storage")
    using_mongodb = False
except Exception as e:
...
def save_feedback(user_id, name, email, message, rating):
    ...
    try:
        print("💾 Using fallback storage for feedback")
        feedback_data = {
            "user_id": user_id,
            "name": name,
            "email": email,
        ...
        if os.path.exists(feedback_file):
            try:
                ...
                print(f"📊 Loaded existing feedback file with {len(all_feedback)} entries")
            except Exception as e:
                print(f"⚠️ Error loading existing feedback file: {str(e)}")
                all_feedback = []
        else:
            print("🆕 Creating new feedback file")
            all_feedback = []
...
def mongo_status():
    """Route to check MongoDB connection status and try different configurations"""
    results = {
        "mongodb_enabled": using_mongodb,
        "mongodb_uri_set": bool(os.environ.get("MONGO_URI")),
        "connection_status": "Not attempted",
        "current_time": str(datetime.now()),
        "environment": os.environ.get("RENDER", "Not on Render"),
        "python_version": sys.version,
        "pymongo_version": "Not checked",
        "certifi_version": "Not checked"
    }
    
    # Only try to connect if MongoDB URI is set
    if os.environ.get("MONGO_URI"):
        try:
            import pymongo
            results["pymongo_version"] = pymongo.__version__
            
            import certifi
            results["certifi_version"] = certifi.__version__
            ...
            for config in connection_configs:
                try:
                    # Create a fresh connection for testing with this config
                    ...
                    results["connection_attempts"].append({
                        "config_name": config["name"],
                        "status": "Success",
                        "options": str(config["options"])
                    })
                    
                    # Store the first successful config
                    if "connection_status" != "Connected successfully":
                        results["connection_status"] = "Connected successfully"
                        results["working_config"] = config["name"]
            ...
            results["connection_status"] = f"Import Error: {str(e)}"
        except Exception as e:
            results["connection_status"] = f"Unexpected Error: {str(e)}"
    
    # Return as HTML for easy viewing
    html = "<h1>MongoDB Connection Status</h1>"
    html += "<table border='1'>"
    ...
    # Add connection attempts table
    if "connection_attempts" in results:
        html += "<h2>Connection Attempts</h2>"
        html += "<table border='1'>"
        html += "<tr><th>Configuration</th><th>Status</th><th>Options</th><th>Error (if any)</th></tr>"
        
        for attempt in results["connection_attempts"]:
            status_color = "green" if attempt["status"] == "Success" else "red"
            html += f"<tr>"
            html += f"<td>{attempt['config_name']}</td>"
            html += f"<td style='color:{status_color}'>{attempt['status']}</td>"
            html += f"<td>{attempt['options']}</td>"
            html += f"<td>{attempt.get('error', '')}</td>"
            html += f"</tr>"
        
        html += "</table>"
        
        # Add a helpful suggestion for updating app.py
        html += "<h2>Next Steps</h2>"
        html += "<p>If a configuration works, update your app.py with similar options:</p>"
        html += "<pre>client = pymongo.MongoClient(\n    MONGO_URI,\n    # Copy the options from the working configuration\n)</pre>"
...
def test_connection():
    try:
        ...
        if MONGO_URI and using_mongodb:
            client = pymongo.MongoClient(MONGO_URI)
            db = client.tr_calculator
            result['connection'] = 'success'
            result['collections'] = db.list_collection_names()
            result['stats'] = db.stats.find_one({"_id": "counter"})
...
def submit_feedback():
    try:
        ...
        print(f"MongoDB URI exists: {bool(MONGO_URI)}")
        ...
        # Use the existing save_feedback function
        success = save_feedback(user_id, name, email, message, rating)
        ...
        if success:
            ...
            return jsonify({'success': False, 'message': 'Unable to save feedback. Please try again later.'})
            
    except Exception as e:
        print(f"Error in submit_feedback: {str(e)}")
        return jsonify({'success': False, 'message': f'Error saving feedback: {str(e)}'})

@app.route('/view-feedback')
def view_feedback():
    """Route to view all feedback for testing purposes"""
    try:
        feedback_data = []
        
        # Try to get feedback from MongoDB
        if using_mongodb:
            try:
                feedback_data = list(feedback_collection.find().sort("timestamp", -1))
                ...
                print(f"Error fetching feedback from MongoDB: {str(e)}")
                # Fall back to file storage
        
        # Try to get feedback from file
        feedback_file = os.path.join(os.getcwd(), 'feedback.json')
        if os.path.exists(feedback_file):
            try:
                with open(feedback_file, 'r') as f:
                ...
                return render_template('feedback_view.html', 
                                     error=f"Error loading feedback: {str(e)}", 
                                     success=False)
        else:
            return render_template('feedback_view.html', 
