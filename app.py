...
try:
    ...
    if MONGO_URI:
        # Connect to MongoDB if URI is provided with proper TLS/SSL configuration
        client = pymongo.MongoClient(
            MONGO_URI,
            tlsCAFile=certifi.where(),  # Use certifi for certificate verification
            connectTimeoutMS=30000,     # Longer timeout
            socketTimeoutMS=30000,      # Longer timeout
...
def mongo_status():
    ...
    if os.environ.get("MONGO_URI"):
        try:
            ...
            results["server_info"] = str(client.server_info())
            
        except ImportError as e:
            results["connection_status"] = f"Import Error: {str(e)}"
        except Exception as e:
            results["connection_status"] = f"Connection Error: {str(e)}"
